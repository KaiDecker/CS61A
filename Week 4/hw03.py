...
def num_eights(n):
    ...
    if n < 10:
        ...
        if n % 10 == 8:
            return num_eights(n // 10) + 1 # 每次遇到8就加1
        else:
...
def knapsack(weights, values, c):
    ...
    if weights == [] or c == 0:
        ...
        with_item = values[0] + knapsack(weights[1:], values[1:], c - weights[0])
        without_item = knapsack(weights[1:], values[1:], c)
...
def move_stack(n, start, end):
    ...
    else: # 这又是典型的分治法，把大问题分成两个小问题
        other = 6 - start - end # 三根柱子编号之和为6
        move_stack(n - 1, start, other) # 先把上面n-1个盘子从start挪到other
        print_move(start, end) # 把最下面的盘子从start挪到end
        move_stack(n - 1, other, end) # 再把other上的n-1个盘子挪到end
