...
def buy(fruits_to_buy: list[str], prices: dict[str, int], total_amount: int) -> None:
    ...
    [9 apples][1 kiwi]
    """
    def add(fruits: list[str], amount: int, cart: str) -> None: # 递归函数，fruits是剩余需要购买的水果种类，amount是剩余金额，cart是当前购物车内容
        if fruits == [] and amount == 0:
            print(cart)
        elif fruits and amount > 0:
            fruit = fruits[0]
            price = prices[fruit]
            for k in range(1, amount // price + 1): # k是购买fruit的数量，且从1开始，确保每种水果至少买一个
